...
bool Compare(RouteModel::Node *n1, RouteModel::Node *n2) {
    return n2->g_value + n2->h_value < n1->g_value + n1->h_value;
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    RouteModel::Node *end_node = current_node->parent;
...
void RoutePlanner::AStarSearch() {
    ...
    while(open_list.size() > 0) {
        auto nxt_node = NextNode();
        if (nxt_node == this->end_node) {
            m_Model.path = ConstructFinalPath(this->end_node);
            break;
        }
        AddNeighbors(nxt_node);
    }
}